...
const stateTabs = [
  ...
  { value: "used", label: "Berisi Karya", icon: FolderOpen },
...
export default function CategoryHero({
  ...
  return (
    <AdminHeroBackground fullWidth>
      <div className="px-4 md:px-6 lg:px-8 pt-8 md:pt-10 3xl:pt-12 4xl:pt-14">
        <div className="flex flex-col items-center text-center sm:flex-row sm:text-left sm:items-start gap-[clamp(0.75rem,0.5rem+1vw,1rem)] sm:gap-5">
          <div className="dashboard-hero-icon flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-cyan-400/10 border border-cyan-400/30 sm:h-16 sm:w-16 3xl:h-20 3xl:w-20 4xl:h-24 4xl:w-24">
            <LayoutList className="h-7 w-7 text-cyan-300 sm:h-8 sm:w-8 3xl:h-10 3xl:w-10 4xl:h-12 4xl:w-12" />
          </div>
          <div className="min-w-0">
            <h1 className="text-[clamp(1.25rem,0.9375rem+1.5vw,1.5rem)] sm:text-3xl 3xl:text-4xl 4xl:text-5xl font-black text-white">
              Kelola <span className="text-cyan-300">Kategori</span>
            </h1>
            <p className="mt-1 text-[clamp(0.8125rem,0.75rem+0.5vw,0.875rem)] max-w-xl text-slate-400 3xl:text-base 4xl:text-lg">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative flex-1 min-w-0 2xl:max-w-2xl 3xl:max-w-3xl 4xl:max-w-4xl">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm text-slate-900 placeholder-slate-400 shadow-sm transition-all focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 sm:py-3 sm:pl-11 3xl:py-3.5 3xl:text-base 4xl:py-4 4xl:pl-12 4xl:text-lg"
            />
          </div>
          <button
            type="button"
            onClick={onAddClick}
            className="flex shrink-0 cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 via-cyan-500 to-blue-600 bg-[length:200%_100%] px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-cyan-500/30 transition-all duration-500 hover:bg-[position:100%_0] sm:py-3 3xl:px-6 3xl:py-3.5 3xl:text-base 4xl:px-7 4xl:py-4 4xl:text-lg"
          >
            <Plus
              size={16}
              className="transition-transform duration-300 group-hover:rotate-90 3xl:h-[18px] 3xl:w-[18px] 4xl:h-5 4xl:w-5"
            />
            Tambah Kategori
          </button>
        </div>
        ...
        <div className="mt-4">
          <div className="relative w-full min-[550px]:hidden">
            <button
              ...
              className="flex w-full cursor-pointer items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-semibold text-slate-200 backdrop-blur-xl transition-all duration-200 hover:bg-white/[0.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/40 sm:py-3 3xl:py-3.5 3xl:text-base 4xl:py-4 4xl:text-lg"
              aria-expanded={isOpen}
              aria-haspopup="listbox"
            >
              <span className="flex min-w-0 items-center gap-2">
                <SelectedIcon className="h-4 w-4 shrink-0 text-cyan-300 4xl:h-5 4xl:w-5" />
                <span className="truncate">{selectedTab.label}</span>
              </span>
              <span className="flex shrink-0 items-center gap-2">
                {stateCounts[selectedTab.value] !== undefined && (
                  <span className="rounded-full bg-cyan-500/20 px-2 py-0.5 text-[11px] tabular-nums text-cyan-300 3xl:text-xs 4xl:text-sm">
            ...
            {stateTabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex min-w-0 cursor-pointer items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-all duration-200 focus-visible:ring-2 focus-visible:ring-cyan-400/40 focus-visible:outline-none 3xl:px-4 3xl:py-2 3xl:text-sm 4xl:px-5 4xl:text-base ${
                    isActive
                ...
                >
                  <Icon
                    className={`h-3.5 w-3.5 shrink-0 transition-colors duration-200 4xl:h-4 4xl:w-4 ${
                      isActive ? "text-white" : "text-slate-500"
                    }`}
                  />
                  {tab.label}
                  {count !== undefined && (
                    <span
                      className={`rounded-full px-1.5 py-0.5 text-[10px] tabular-nums 3xl:text-[11px] 4xl:text-xs ${
                        isActive
