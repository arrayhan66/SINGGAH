  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-5 md:gap-6 lg:gap-7 3xl:gap-8 4xl:gap-9">
      <div className="flex flex-col gap-3 min-[500px]:flex-row min-[500px]:items-center min-[500px]:gap-4 lg:gap-5 3xl:gap-6">
        <div className="group flex flex-1 min-w-0 items-center gap-2.5 rounded-xl border-2 border-white/60 bg-white/90 px-3.5 py-2.5 backdrop-blur-md shadow-lg shadow-black/20 transition-all duration-300 hover:shadow-xl focus-within:-translate-y-1 focus-within:border-cyan-400 focus-within:bg-white focus-within:shadow-cyan-400/30 sm:gap-3 sm:rounded-2xl sm:px-5 sm:py-3 md:py-3.5 lg:py-4 3xl:gap-4 3xl:px-6 3xl:py-4.5 4xl:py-5">
          <Search
            size={16}
      ...
      {!isDosen && (
        <div className="relative min-w-0" ref={dropdownRef}>
          <button
            ...
            className="flex w-auto cursor-pointer items-center justify-between gap-2.5 rounded-xl border-2 border-white/60 bg-white/90 px-4 py-2.5 text-sm font-medium text-slate-800 backdrop-blur-md shadow-lg shadow-black/20 transition-all duration-300 hover:shadow-xl focus-within:-translate-y-1 focus-within:border-cyan-400 focus-within:bg-white focus-within:shadow-cyan-400/30 sm:rounded-2xl sm:px-5 sm:py-3 md:py-3.5 lg:py-4 3xl:gap-3 3xl:px-6 3xl:py-4.5 4xl:py-5"
          >
          ...
          {isOpen && (
            <div className="absolute left-0 z-30 mt-2 w-[220px] overflow-hidden rounded-xl border-2 border-white/10 bg-slate-900/95 shadow-2xl shadow-cyan-900/20 backdrop-blur-xl sm:rounded-2xl">
              {options.map((option) => (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => handleSelect(option.value)}
                  className={`flex w-full cursor-pointer items-center gap-2 px-3.5 py-2.5 text-sm font-medium text-left transition-colors sm:px-4 sm:py-3 3xl:px-4 3xl:py-3.5 4xl:py-4 ${
                ...
                >
                  ...
                  {statusFilter === option.value && (
                    <Check size={15} className="shrink-0 text-cyan-400 sm:size-4 3xl:size-[18px] 4xl:size-5" />
                  )}
                </button>
              ))}
            </div>
