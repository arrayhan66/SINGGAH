...
function EmojiPicker({
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div
          className={`animate-modal-in absolute z-50 w-72 max-w-[calc(100vw-1.25rem)] overflow-hidden rounded-2xl border border-white/10 bg-slate-900/95 shadow-2xl shadow-black/60 ring-1 ring-cyan-400/10 backdrop-blur-2xl sm:w-80 ${
        ...
        >
          {/* Header + Pencarian */}
          <div className="border-b border-white/10 bg-white/[0.04] px-3 py-2.5">
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-white/10 bg-white/5 py-1.5 pl-8 pr-3 text-xs text-white placeholder-slate-500 outline-none transition focus:border-cyan-400/50 focus:bg-white/10"
                />
                {query && (
                  <button
                    type="button"
                    onClick={() => setQuery("")}
                    className="absolute right-2 top-1/2 -translate-y-1/2 cursor-pointer rounded-full p-0.5 text-slate-500 transition hover:bg-white/10 hover:text-white"
            ...
            <div className="flex items-center gap-1 px-3 pb-2">
              {EMOJI_CATEGORIES.map((cat) => (
                <button
                  ...
                  title={cat.label}
                  className={`flex h-7 flex-1 cursor-pointer items-center justify-center rounded-lg text-base transition ${
                    activeCat === cat.id
          ...
          {/* Grid emoji */}
          <div className="scrollbar-thin scrollbar-thumb-white/10 max-h-44 overflow-y-auto px-3 pb-3">
          ...
          {/* Preview */}
          <div className="flex items-center gap-2.5 border-t border-white/10 bg-white/[0.04] px-3 py-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-cyan-500/10 text-xl ring-1 ring-cyan-400/20">
              {preview?.e || "😀"}
            </span>
